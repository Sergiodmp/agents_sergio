...
def order_budget(budget, code):
    df = pd.DataFrame()
    df_agents = pd.read_csv(f'agents.csv', header=0, delimiter=",", \
                            engine='python')
...
def auction_blank_df():
    ...
    df = pd.DataFrame([], columns=['id', 'agent_type', 'location_1', 'location_2',\
                    'location','coil_auction_winner', 'coil_length', 'coil_width', \
                    'coil_thickness', 'coil_weight','int_fab', 'bid', 'budget', \
    ...
    if agent_name == 'ca':
        ...
        agent_data = agent_data.reindex(
            columns=['id', 'agent_type', 'location_1', 'location_2', \
                     ...
                     'int_fab', 'bid', 'bid_status', 'coil_length', \
                     'coil_width', 'coil_thickness', 'coil_weight', \
...
def coil_parameters(agent_data, agents_df, agent_name):
    ...
    agent_data.at[0, 'coil_weight'] = agent_data.at[0, 'coil_length'] * \
                    agent_data.at[0, 'coil_width'] * agent_data.at[0, \
                    'coil_thickness'] * (1/1000) * (1/100) *(1/ 7850)
    #
    # between 10-10.5 m/s. Fab takes between 8 and 10 min with
    # this conditions. process time = length / speed
    agent_data.at[0, 'setup_speed'] = 10 + (rn/2)
    agent_data.at[0, 'T1'] = 250 + (rn*100)  # between 250-350
    agent_data.at[0, 'T2'] = 550 + (rn*100)  # between 550-650
    agent_data.at[0, 'T3'] = 800 + (rn*100)  # between 800-900
    agent_data.at[0, 'T4'] = 600 + (rn*100)  # between 600-700
    ...
    #
    # Planning: between now and in 40 min.
    agent_data.at[0, 'ship_date'] = random_date(datetime.datetime.now(), \
...
#   Power [kw] =     a + Width-Out (VA) * b + Final Thickness * c + Tin Layer Up * d + Tin Layer Down * e + Speed (VA) * f [kw]  
    production_time = process_df['processing_time'] /60                                          # Production time of the coil at the finishing line (min)
    energy_demand = power * production_time / 60                                                 # KWh
    energy_cost= energy_demand* price_energy_consumption                                               
    return energy_cost
######################

def va_bid_evaluation(coil_msgs_df, va_data_df, step, price_energy_consumption, df_parameters_energy, process_df):
    key = []
    ...
    for i in range(coil_msgs_df.shape[0]):
        ...
        coil_msgs_df.at[i, 'energy_cost'] =  energy_cost(df_parameters_energy, va_data_df, process_df, price_energy_consumption) 
    ...
    for i in range(coil_msgs_df.shape[0]):
        ...
        coil_msgs_df.loc[i, 'minimum_price'] = m + n + j
    for i in range(coil_msgs_df.shape[0]):
...
def auction_va_kpis(agent_df, coil_msgs_df, auction_df, process_df,\
    ...
    df.at[0, 'coil_length'] = coil_msgs_df.loc[0, 'largo']
    df.at[0, 'coil_width'] = coil_msgs_df.loc[0, 'ancho']
    df.at[0, 'coil_thickness'] = coil_msgs_df.loc[0, 'espesor']
