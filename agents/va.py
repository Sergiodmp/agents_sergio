    ...
    class VA_Behav(CyclicBehaviour):
        async def run(self):
            global process_df, va_status_var, my_full_name, coil_notified, \
                   ...
                   rep_pauction, all_auctions, df_parameters_energy, price_energy_consumption
            #df_parameters_energy added 25 feb
            ...
            if va_status_var == "auction":
                ...
                if self.act_qry('counterbid','auction') == 0 and \
                    ...
                    if len(jid_list) > 0:
                        bid_coil = asf.va_bid_evaluation(coil_msgs_df, va_data_df,'bid',price_energy_consumption, df_parameters_energy, process_df)
                            ...
                            if len(globals.tosend) == 0 or dlt > 15:
                                ...
                                if coil_msgs_df2.shape[0] > 0:
                                    ...
                                    counterbid_coil = asf.va_bid_evaluation(coil_msgs_df2, \
                                                va_data_df,'counterbid',price_energy_consumption, df_parameters_energy, process_df)
            ...
            if va_status_var == "post-auction":
                ...
                if coil_notified < (coil_msgs_df2.shape[0]-1) and pending_au and \
                    ...
                    coil_jid_winner_f= results_2.loc[i,'Coil']                          #   columnas de results_2: 'Coil', 'Minimum_price', 'Bid', 'Difference', 'Budget_remaining','Counterbid','Profit']
                    coil_jid_winner  = coil_jid_winner_f.split('@')[0]
                    winner_df = results_2.loc[i:i,:]                                    # dataframe no global que almacena los datos de la ganadora
                    winner_df = winner_df.reset_index(drop=True)
                    all_winner_df=pd.concat([all_winner_df,winner_df])                  #sergio 02 marzo. Dataframe con datos de todos los ganadores

                    # se podría hacer un dataframe que almacene las filas que se generan en ese winner cada vez
                    ...
                    if recl is not None: # Message from coil agents
                        ...
                        if cl_df.loc[0, 'purpose'] == 'OKacceptedbid':
                            ...
                            auction_df.at[0, 'coil_ratings'] = [coil_msg_df3.to_dict(\
                                    orient="records")]  # Save information to auction df. Almacena la info de la ganadora.
...
if __name__ == "__main__":
    ...
    index_va = ['va9', 'va10', 'va11', 'va11', 'va12', 'va12']
